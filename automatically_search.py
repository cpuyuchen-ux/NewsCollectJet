...
2.2rem; font-weight: 700; color: #1F2937; margin-bottom: 0.2rem; }
    .sub-header { color: #6B7280;
font-size: 1.0rem; margin-bottom: 0.8rem; }
    .warning-bar {
...
st.markdown('<div class="sub-header">自動整合 Google News 即時新聞，並透過 Gemini AI 自動進行年份對齊與格式化</div>',
unsafe_allow_html=True)
 
st.markdown("""
<div class="warning-bar">
    <p
class="warning-text">※此系統為個人自主開發，請勿用做非法行為😈</p>
    <p
class="warning-text">※檢索資料庫為「彰化家扶」常見出報媒體，資料庫將不定期更新👀</p>
    <p
class="warning-text">※此系統供同工免費使用，惟開發者仍保有此系統所有權🔧</p>
</div>
""", unsafe_allow_html=True)
...
if not api_key:
    api_key =
st.sidebar.text_input("🔑 輸入 Gemini API Key:", type="password", help="請輸入您的 Gemini API Key")
...
if os.path.exists(db_file_path):
    try:
        db_df =
pd.read_csv(db_file_path, encoding='utf-8').dropna(how='all')
        st.sidebar.success("✅
database.csv 已連線")
        if len(db_df.columns) >=
2:
            media_col =
db_df.columns[0]
            type_col =
db_df.columns[1]
            media_type_map =
dict(zip(db_df[media_col].astype(str).str.strip(),
...
margin-top: 10px; margin-bottom: 20px;">
        <div
style="font-size: 0.9rem; font-weight: 600; color: #374151; margin-bottom:
5px;">{text}</div>
        <div style="width:
100%; background-color: #E5E7EB; border-radius: 10px; height: 24px; position:
relative; overflow: hidden;">
            <div
style="width: {percent}%; background: linear-gradient(90deg, #3B82F6,
#1D4ED8); height: 100%; transition: width 0.4s ease; display: flex;
...
style="font-size: 14px;">🛫</span>
        ...
        <div
style="text-align: right; font-size: 0.85rem; font-weight: 700; color:
#2563EB; margin-top: 3px;">🛫 {percent}%</div>
    </div>
    """
...
def run_news_pipeline(office, staff_name, org, keyword, year, media_map,
GEMINI_API_KEY):
...
urllib.parse.quote(search_query)
    rss_url =
f"https://news.google.com/rss/search?q={encoded_query}&hl=zh-TW&gl=TW&ceid=TW:zh-Hant"
   
    raw_results = []
    with st.spinner(f"📡 正在經由 Google News 檢索『{search_query}』新聞報導..."):
        try:
            req =
urllib.request.Request(rss_url, headers={'User-Agent': 'Mozilla/5.0 (Windows NT
10.0; Win64; x64)'})
            with
...
item.find('pubDate').text if item.find('pubDate') is not None else ""
                source =
item.find('source').text if item.find('source') is not None else "新聞媒體"
               
               raw_results.append({"title": title, "url": link,
"date": pub_date, "media_name": source})
        except Exception as e:
            st.error(f"❌ Google
News 檢索異常：{e}")
            return []
...
genai.Client(api_key=GEMINI_API_KEY)
    ...
    batch_size = 5
    batches = [raw_results[i:i +
batch_size] for i in range(0, len(raw_results), batch_size)]
...
item["date"], "media_name": item["media_name"]}
            for i, item in
enumerate(batch)
        ]
        ...
        傳回 JSON 格式：
        {{"articles":
[{{"id": 0, "media_name": "媒體名稱", "title": "純標題",
"reporter": "記者姓名"}}]}}
        """
...
art_id < len(batch):
                        m_name =
art.get("media_name") or batch[art_id]["media_name"]
                       results.append({
                           "media_name": m_name,
...
batch[art_id]["title"]),
        ...
        if not success:
            ...
            for item in batch:
                c_title =
clean_title_local(item["title"], item["media_name"])
                results.append({
                   "media_name": item["media_name"],
                   "media_type": lookup_media_type(item["media_name"],
media_map),
                   "title": c_title,
                   "reporter": "編輯部",
                    "url":
item["url"]
        ...
        current_pct = int((idx /
len(batches)) * 100)
       progress_placeholder.markdown(render_airplane_progress(current_pct,
f"🛫 正在處理第 {idx}/{len(batches)} 批次..."), unsafe_allow_html=True)
        time.sleep(1)
...
if sidebar_option == "主控台 / 檢索系統":
    ...
    with st.container():
        st.markdown('<div
class="search-card">', unsafe_allow_html=True)
        st.subheader("🔍 設定檢索條件")
...
st.columns(2)
        with row1_col1:
            selected_office =
st.selectbox("🏢 篩選服務處", ["全部", "和美兒童館",
"員林服務處", "田中服務處", "彰化服務處",
"二林服務處", "中心行政組"])
        with row1_col2:
            staff_name =
st.text_input("👤 同工姓名", placeholder="e.g. 家扶小幫手")
 
        # 第二排：改為浮水印（placeholder）提示
        row2_col1, row2_col2,
row2_col3 = st.columns(3)
        with row2_col1:
            target_org =
st.text_input("🏢 機構 / 品牌名稱", placeholder="e.g. 彰化家扶")
        with row2_col2:
            search_keyword =
st.text_input("🔑 搜尋關鍵字", placeholder="e.g. 課輔班、相見歡")
        with row2_col3:
            target_year =
st.text_input("📅 目標年份 (YYYY)", placeholder="e.g. 2026")
 
       st.markdown("</div>", unsafe_allow_html=True)
 
    if st.button("🚀 開始自動化檢索與解析", type="primary", use_container_width=True):
        if not staff_name or not
target_org or not search_keyword or not target_year:
            st.error("⚠️ 請完整填寫所有欄位條件！")
        else:
            results =
run_news_pipeline(selected_office, staff_name, target_org, search_keyword,
target_year, media_type_map, api_key)
...
"新聞連結"]
               ...
               st.dataframe(df_export, column_config={"新聞連結": st.column_config.LinkColumn("新聞連結")},
...
writer.sheets['輿情報導']
                    for row_idx, url
in enumerate(df_export['新聞連結'], start=2):
                        cell =
worksheet.cell(row=row_idx, column=7)
                       cell.hyperlink = url
                        cell.style =
"Hyperlink"
